...
protected:
  ...
  actionlib::SimpleActionServer<action_tut::ExampleAction> as_; 
  std::string action_name_;
  // create messages that are used to published feedback/result
  action_tut::ExampleFeedback feedback_;
  action_tut::ExampleResult result_;
...
public:
  ...
  void executeCB(const action_tut::ExampleGoalConstPtr &goal)
  {
    // helper variables
    ros::Rate r(1);
    bool success = true;

    // push_back the seeds for the test sequence
...
int main(int argc, char** argv)
{
  ...
  ExampleAction test(ros::this_node::getName());
  ros::spin();
